...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (!activeSession && selectedSession) {
      setQuickSession(selectedSession);
    }
  ...
  const handleSell = async (productId: string) => {
    ...
    setMessage(`${product?.name ?? "商品"}を記録しました`);
    window.setTimeout(() => setFlashProductId(""), 260);
            ...
            <div className="flex flex-col gap-4 xl:flex-row xl:items-end">
              <div className="grid flex-1 gap-3 md:grid-cols-4">
                <label className="text-sm font-bold text-slate-600">
                  営業名
                  <input value={quickSession.name} onChange={(event) => setQuickSession({ ...quickSession, name: event.target.value })} className="mt-1 w-full rounded-md border border-line bg-white p-3 text-slate-950" />
                </label>
                <label className="text-sm font-bold text-slate-600">
                  日付
                  <input type="date" value={quickSession.date} onChange={(event) => setQuickSession({ ...quickSession, date: event.target.value })} className="mt-1 w-full rounded-md border border-line bg-white p-3 text-slate-950" />
                </label>
                <label className="text-sm font-bold text-slate-600">
                  場所
                  <input value={quickSession.location} onChange={(event) => setQuickSession({ ...quickSession, location: event.target.value })} className="mt-1 w-full rounded-md border border-line bg-white p-3 text-slate-950" placeholder="任意" />
                </label>
                <label className="text-sm font-bold text-slate-600">
                  売上目標
                  <input type="number" value={quickSession.targetSales} onChange={(event) => setQuickSession({ ...quickSession, targetSales: Number(event.target.value) })} className="mt-1 w-full rounded-md border border-line bg-white p-3 text-slate-950" />
                </label>
              </div>
        ...
        <div className="rounded-lg border border-line bg-panel p-4">
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => void closeActiveSession()} disabled={!activeSession} className="rounded-md bg-danger px-4 py-3 font-black text-white disabled:bg-slate-700">
                収店
              </button>
            </div>
          </div>
              ...
              <section key={category.id}>
                ...
                <div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-4">
                  {categoryProducts.map((product) => (
                    <ProductButton key={product.id} product={product} onSell={handleSell} onLongPress={setStockProduct} activeFlash={flashProductId === product.id} />
                  ))}
                </div>
              </section>
            );
              ...
              .map((item) => (
                <button key={item.id} onClick={() => setBundle(item)} className="rounded-lg border border-line bg-white p-4 text-left hover:border-mint active:scale-[0.98]">
                  ...
                  <p className="mt-1 text-sm text-slate-500">
                    串 {item.itemCount}本{item.includesDrink ? " + ドリンク" : ""} / 値引 {yen(item.discountAmount)}
                  </p>
                </button>
              ))}
          </div>
